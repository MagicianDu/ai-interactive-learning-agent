...
If `learning_agent.prepare_content_review` returns `revision_required`, Codex should read the brief, inspect `currentMetrics` and `automaticFindings`, critique and revise the bundle, call `learning_agent.publish_learning_course` again, then call `learning_agent.record_content_review_report` with reviewer verdict, concrete issues, and round number. Use the returned metrics and delta to confirm that review is actually reducing template labels, missing imagegen assets, generic titles, low-density pages, generic sourceTrace supports, stale visual prompts, title-duplicating image prompts, and weak mechanism-depth pages. Repeat until the third round completes or the tool returns `review_complete`. If `learning_agent.validate_imagegen_assets` returns `failed`, regenerate or record the affected page images before returning the preview. `learning_agent.calibrate_learning_course` remains available for targeted quality-report-driven fixes, but the default self-study quality path is now content review plus imagegen batch validation.
