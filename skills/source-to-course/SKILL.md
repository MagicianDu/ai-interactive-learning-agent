...
For source-backed `student_self_study_textbook`, use the productized review and imagegen batch flow: run three Codex content-review rounds with `learning_agent.prepare_content_review`, record each round with `learning_agent.record_content_review_report`, then generate the imagegen manifest, record every generated PNG/WebP, validate assets, and run layout smoke when the dev server is available before showing the final preview. The learner should not approve review briefs, review reports, image manifests, or layout reports.
...
If `learning_agent.prepare_content_review` returns `revision_required`, Codex should read the brief, inspect `currentMetrics` and `automaticFindings`, critique and revise the bundle, call `learning_agent.publish_learning_course` again, then call `learning_agent.record_content_review_report` with reviewer verdict, concrete issues, and round number. Use the returned metrics and delta to confirm that review is actually reducing template labels, missing imagegen assets, generic titles, low-density pages, generic sourceTrace supports, stale visual prompts, title-duplicating image prompts, weak mechanism-depth pages, boilerplate learner phrases, and repeated board section labels. Repeat until the third round completes or the tool returns `review_complete`. If `learning_agent.validate_imagegen_assets` returns `failed`, regenerate or record the affected page images before returning the preview. When a dev server is running, execute `npm run smoke:layout -- --runId <run-id> --desktop-only`; if it fails, fix page scroll, missing images, console errors, or content overflow before returning the preview. `learning_agent.calibrate_learning_course` remains available for targeted quality-report-driven fixes, but the default self-study quality path is now content review plus imagegen batch validation plus layout smoke.
