...
When `learning_agent.next_course_production_action` returns `run_content_review`, call `learning_agent.prepare_content_review`, revise the bundle as `content-review-agent`, republish, and call `learning_agent.record_content_review_report`. When it returns `generate_imagegen_assets` or `fix_imagegen_assets`, call imagegen for each pending item and record results with `learning_agent.record_imagegen_batch_item`. When it returns `run_layout_smoke`, run the provided command. When it returns `fix_layout`, revise the affected pages and rerun smoke. `learning_agent.calibrate_learning_course` remains available for targeted quality-report-driven fixes, but the default self-study quality path is now the one-shot production pipeline.
