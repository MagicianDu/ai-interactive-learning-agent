...
describe("Codex MCP tool profile check", () => {
  test("accepts the pruned learner default and explicit advanced profiles", () => {
    expect(() =>
  ...
  test("requires authoring and operator tools through explicit profiles", () => {
    expect(() =>
      validateMcpProfileToolLists({
        learner: learnerTools,
        authoring: ["learning_agent.prepare_learning_course"],
        operator: ["learning_agent.prepare_learning_course"]
      })
