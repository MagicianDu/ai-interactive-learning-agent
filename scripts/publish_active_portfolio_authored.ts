...
const runId = "active-portfolio-book-authored-entry-v1";

async function main(): Promise<void> {
  const tools = new LearningAgentRuntimeTools(process.cwd());
  const context = JSON.parse(
    await readFile(`runs/${runId}/artifacts/authoring-context.v1.json`, "utf8")
  ) as AuthoringContext;

  const coursePack = {
    id: runId,
    title: "主动投资组合管理：前三章自学课程包",
    parentRunId: runId,
    ...
    units: context.coursePlan.recommendedUnits.map((unit, index) => ({
      unitId: unit.unitId,
      title: unit.title,
      kind: unit.kind,
  ...
  const lessons = context.coursePlan.recommendedUnits.map((unit) => buildLesson(context, unit));
  const result = await tools.callTool("learning_agent.publish_learning_course", {
    runId,
    coursePack,
    lessons,
    publishNotes: "按章节驱动发布《主动投资组合管理》前三章中文自学课程预览。"
  });
...
function buildVisualSpec(
  ...
  return {
    ...
    imagePrompt: `为中文学术自学 Web Deck 生成教学插图，主题是：${page.title}。核心命题：${page.core}。画面只服务于知识理解，可使用少量短标签帮助辨认结构、对比或因果；不要复写页面标题、底部总结或大段正文；不要表格、不要 UI 面板、不要整页卡片、不要长段落文字。`
  };
}
