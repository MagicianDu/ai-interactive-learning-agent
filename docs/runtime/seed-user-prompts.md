...
请调用 learning_agent.prepare_learning_course 获取来源锚点、推荐单元和发布约束。然后由 Codex 创作 coursePack 与 lessons，并调用 learning_agent.publish_learning_course 发布中文网页。
...
不要让我审批内部 artifacts。请调用 learning_agent.prepare_learning_course 获取作者上下文，然后由 Codex 创作 course bundle 并调用 publish_learning_course。
```
...
默认不要进入专家审核流程；请调用 learning_agent.prepare_learning_course 后由 Codex 创作课程，再调用 learning_agent.publish_learning_course 发布可预览网页。
```
...
## Source-Backed Fast Preview -> generate_grounded_course
